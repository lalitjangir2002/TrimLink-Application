...
const DashboardLayout = () => {
    ...
    const { isLoading: isMyUrlsLoading, data: myShortenUrls, refetch } = useFetchMyShortUrls(token, onError);
    // console.log(myShortenUrls)

    const { isLoading: isTotalClicksLoading, data: totalClicks } = useFetchTotalClicks(token, onError);

    return (
        <div className="lg:px-14 sm:px-8 px-4 min-h-[calc(100vh-64px)]">
            {isTotalClicksLoading || isMyUrlsLoading ? (
                <Loader />
            ) : (
